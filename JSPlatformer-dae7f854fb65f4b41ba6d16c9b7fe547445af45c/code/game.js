  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Because there is a third case (space ' '), use an "else if" instead of "else"
        ...
        fieldType = "lava";
    //Allows players to jump into and will stick to that region but cannot jump
      else if (ch == "y")
        fieldType = "velcro";
...
Level.prototype.obstacleAt = function (pos, size) {
 var xStart = Math.floor(pos.x); // 4.999999
 var xEnd = Math.ceil(pos.x + size.x); // 4.1 -> 5
 var yStart = Math.floor(pos.y);
 var yEnd = Math.ceil(pos.y + size.y);

 // Consider sides, top, and bottom as walls
